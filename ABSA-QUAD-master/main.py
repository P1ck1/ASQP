...
def evaluate(data_loader, model, sents):
    ...
    model.model.to(device)  # 将模型移到指定的GPU上

    model.model.eval()  # 将模型切换到评估模式
    ...
    for batch in tqdm(data_loader):
        ...
        # 对输出解码，解码成释义文本。target也一样要解码
        dec = [tokenizer.decode(ids, skip_special_tokens=True) for ids in outs]
        target = [tokenizer.decode(ids, skip_special_tokens=True) for ids in batch["target_ids"]]
        # 将每个批次的生成结果 dec 和真实标签 target 分别存入 outputs 和 targets 列表中
        outputs.extend(dec)
    ...
    # 记住outputs是预测输出，targets是目标，都是自然语言序列
    scores, all_labels, all_preds = compute_scores(outputs, targets, sents)  # 这个sents传过去了，但其实并没有被用到。
...
if args.do_train:
    ...
    # 初始化一个 PyTorch Lightning 的 Trainer 实例。Trainer 是 PyTorch Lightning 中用于训练的核心类。
...
if args.do_direct_eval:
    ...
    # 至此，这里存储的数据已经是编码向量了，不是最初的字符了
    # print(test_loader.device)

    # compute the performance scores
    # 把这个数据集传过去，之后，在evaluate函数中会使用model自己计算输出与target,这个sents传过去其实没有用到
    # 所以这里就是只要传数据集就好了，不用管复杂的
